...
(function () {
  ...
  function renderDist(dist, curTries) {
  ...
  async function loadArchive(di) {
    ...
    if (archiveOnline()) {
      ...
      let res;
      try { res = await window.ZOZDLE_ONLINE.archiveStatus(diToISO(di)); }
      catch { return toast("Couldn't load that day", true); }
      if (!res || res.error) return toast("Couldn't load that day", true);
      const evals = (res.patterns || []).map(patToEval);
      state = {
        mode: "archive", server: true, archiveDate: diToISO(di), di,
