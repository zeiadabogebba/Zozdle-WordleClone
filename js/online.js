...
(function () {
  ...
  async function loadProfile() {
    if (!O.user) { O.profile = null; O.dist = null; return; }
    const [{ data: prof, error }, { data: plays }] = await Promise.all([
      sb.from("profiles").select("username,current_streak,max_streak,total_wins,total_played").eq("id", O.user.id).single(),
      sb.from("plays").select("solved,tries"),
    ]);
    if (error) console.warn("Zozdle: profile load failed —", error.message);
    O.profile = prof || null;
    const dist = {};
    for (const p of plays || []) if (p.solved && p.tries != null) dist[p.tries] = (dist[p.tries] || 0) + 1;
    O.dist = dist;
  }
  ...
  const VAPID = (window.ZOZDLE_PUSH && window.ZOZDLE_PUSH.vapidPublic) || "";
  O.pushSupported = !!VAPID && !VAPID.includes("YOUR-") && "Notification" in window && "serviceWorker" in navigator && "PushManager" in window;
  ...
  async function ensureSubscribed() {
    ...
    await sb.from("push_subscriptions").upsert(
      { user_id: O.user.id, endpoint: j.endpoint, p256dh: j.keys.p256dh, auth: j.keys.auth, tz: tzGuess(), updated_at: new Date().toISOString() },
      { onConflict: "endpoint" }
    );
    return true;
  }

  O.loadNotifyPrefs = async () => {
    if (!O.user) return { notify_new: false, notify_before: false };
    const { data } = await sb.from("notify_prefs").select("notify_new,notify_before").eq("user_id", O.user.id).maybeSingle();
    return data || { notify_new: false, notify_before: false };
  };

  O.enablePush = async () => {
    if (!O.user) return false;
    try { return await ensureSubscribed(); } catch { return false; }
  };

  O.setNotifyPref = async (patch) => {
    if (!O.user) return { error: "auth" };
    const { error } = await sb.from("notify_prefs").upsert(
      { user_id: O.user.id, tz: tzGuess(), updated_at: new Date().toISOString(), ...patch },
      { onConflict: "user_id" }
    );
    return error ? { error: "network" } : { ok: true };
  };
  O.myPlays = async () => { const { data, error } = await sb.from("plays").select("puzzle_date,solved,finished,tries"); if (error) throw error; return data || []; };
  O.archiveStatus = async (date) => { const { data, error } = await sb.rpc("archive_status", { p_date: date }); if (error) throw error; return data; };
  O.submitArchiveGuess = async (g, date) => { const { data, error } = await sb.rpc("submit_archive_guess", { p_guess: g, p_date: date }); return error ? { error: "network" } : data; };
  ...
  function lbTable(rows) {
    return `<div class="lb">` + rows.map((r, i) =>
      `<div class="lb-row${O.profile && r.username === O.profile.username ? " me" : ""}">
         ...
         <span class="lb-pts">✦ ${r.points != null ? r.points : 0}</span>
       </div>`).join("") + `</div>`;
  }
