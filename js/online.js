...
(function () {
  ...
  O.ready = new Promise((resolve) => {
    let done = false;
    // IMPORTANT: never await Supabase calls *inside* onAuthStateChange — the callback
    // runs while the auth lock is held, so any query (loadProfile, RPCs…) would
    // deadlock and hang forever ("Loading…"). Defer the work with setTimeout(0).
  ...
  // archive: the user's play history (for the calendar) + play/inspect any past date
