-- ============================================================
--  Local-midnight daily: the daily word now flips at each player's
--  local midnight (keyed by their calendar date) instead of 00:00 UTC.
--  The client passes its local date; the server clamps it to ±1 day of
--  UTC so nobody can jump to an arbitrary day. Everyone on the same
--  calendar date still gets the same word.
--
--  Run once in the Supabase SQL editor. (0001_init.sql is updated too,
--  for fresh installs.) The function signatures change, so drop first.
-- ============================================================
