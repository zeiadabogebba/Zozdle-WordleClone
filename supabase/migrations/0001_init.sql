...
create table if not exists public.notify_prefs (
  ...
  notify_before boolean not null default false,
  tz            text,
  updated_at    timestamptz default now()
);
alter table public.notify_prefs enable row level security;
drop policy if exists "own notify prefs" on public.notify_prefs;
...
create or replace function public.update_streak(p_uid uuid, p_date date, p_solved boolean, p_tries int default null)
returns void language plpgsql security definer set search_path = public as $$
declare pr record;
begin
  ...
  if p_solved then
    update profiles set
      ...
      total_points     = pr.total_points + coalesce(7 - p_tries, 0)
    where id = p_uid;
...
begin
  ...
  if is_finished then
    perform update_streak(uid, d, is_solved,
      case when is_solved then array_length(new_patterns, 1) else null end);
  end if;
...
returns table (username text, streak int, max_streak int, wins int, points int)
language sql stable security definer set search_path = public as $$
  select username,
    case when last_solved_date >= (now() at time zone 'utc')::date - 1
         then current_streak else 0 end as streak,
    max_streak, total_wins, total_points
  from profiles
  order by streak desc, total_points desc, max_streak desc, total_wins desc
  limit p_limit;
$$;
...
language sql stable security definer set search_path = public as $$
  select p.username,
    ...
    p.max_streak, p.total_wins, p.total_points
  from league_members m
  join profiles p on p.id = m.user_id
  where m.league_id = p_league and public.is_member(p_league)
  order by streak desc, p.total_points desc, p.max_streak desc, p.total_wins desc;
$$;
...
begin
  ...
  if is_finished then
    update profiles set
      ...
      total_points = total_points + (case when is_solved then 7 - array_length(new_patterns, 1) else 0 end)
    where id = uid;
  end if;
...
grant execute on function
  ...
  public.archive_status(date), public.submit_archive_guess(text, date)
  to authenticated;
